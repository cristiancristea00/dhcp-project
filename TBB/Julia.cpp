...
auto main(int const argc, char * argv[]) -> int
{
    ...
    std::cout << std::format("Generating Julia fractal image with size {}×{} using {} iterations and grainsize {}×{} on {} threads\n", imageWidth, imageHeight, maxIterations, grainsizeRow, grainsizeCol, numThreads);

    JuliaGenerator juliaGenerator{imageSize, maxIterations};
    TestSpeed(
        [&juliaGenerator, &grainsize]() -> void
        {
            CALLGRIND_ZERO_STATS;
            CALLGRIND_START_INSTRUMENTATION;

            juliaGenerator.render(grainsize);
