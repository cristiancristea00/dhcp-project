...
auto main(int const argc, char * argv[]) -> int
{
    ...
    std::cout << std::format("Generating Mandelbrot fractal image with size {}×{} using {} iterations and grainsize {}×{} on {} threads\n", imageWidth, imageHeight, maxIterations, grainsizeRow, grainsizeCol, numThreads);

    MandelbrotGenerator mandelbrotGenerator{imageSize, maxIterations};
    TestSpeed(
        [&mandelbrotGenerator, &grainsize]() -> void
        {
            CALLGRIND_ZERO_STATS;
            CALLGRIND_START_INSTRUMENTATION;

            mandelbrotGenerator.render(grainsize);
